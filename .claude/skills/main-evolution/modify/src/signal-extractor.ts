...
export function extractMainSignals(options: {
  ...
  try {
    ...
    // 组件特定信号增强
    if (component) {
      const componentSignals = extractComponentSpecificSignals(content, component, language);
    ...
    logger.warn({ error }, 'Main project signal extraction failed, returning empty array');
    return [];
  }
}
...
function extractComponentSpecificSignals(
  ...
  // 组件特定的信号模式
  const componentPatterns: Record<MainComponent, Array<{
    type: SignalType;
    patterns: RegExp[];
    weight: number;
    actionable: boolean;
