...
export class SearchLearningExecutor {
   ...
   * 子代理并行执行搜索和学习，比较新旧方法
   */
  async searchAndCompareMethods(
    prompt: string,
    groupFolder: string,
  ): Promise<string> {
    // 使用 Promise.all 并行执行搜索任务
    const [memoryResults, evolutionResults, externalResults] =
      await Promise.all([
    ...
    // 归纳总结搜索结果
    const findings = this.summarizeFindings(
      memoryResults,
      evolutionResults,
      externalResults,
    );
  ...
  private summarizeFindings(
    ...
    if (
      externalResults.clawhub.length > 0 ||
      externalResults.skillsSh.length > 0
    ) {
      summary += '\n### 外部资源中的方法：\n';
  ...
  private async evaluateMethods(
    ...
    const hasMultipleMethods =
      (hasMemoryMethods ? 1 : 0) +
        (hasEvolutionMethods ? 1 : 0) +
        (hasExternalMethods ? 1 : 0) >=
      2;
