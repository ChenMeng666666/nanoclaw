...
// Mock db-routing (must come before db-agents due to import order)
vi.mock('../db-routing.js', () => ({
  createRoutingBinding: vi.fn(),
  getRoutingBinding: vi.fn(() => null),
}));

// Mock db-agents - return test data for connect() to work
const mockAgent = {
  id: 'agent_test',
  name: 'TestAgent',
  folder: 'test-agent',
  isActive: true,
  credentials: { anthropicModel: 'claude-sonnet-4-6' },
};
...
vi.mock('../db-agents.js', () => ({
  getAllActiveAgents: vi.fn(() => [mockAgent]),
  getChannelInstancesForAgent: vi.fn(() => [mockChannelInstance]),
  getChannelInstanceByJid: vi.fn((jid) => {
    if (jid === 'tg:100200300') return mockChannelInstance;
    if (jid === 'tg:-1001234567890') {
      return {
        ...mockChannelInstance,
        jid: 'tg:-1001234567890',
        botId: 'test_bot_token_2',
      };
    }
    return undefined;
  }),
