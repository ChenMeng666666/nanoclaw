...
NanoClaw 实现了三级分层记忆系统，模拟人类记忆的工作方式：
...
| `/api/memory/search` | POST | 搜索记忆（支持语义检索） |
| `/api/memory/get/:id` | GET | 获取特定记忆 |
| `/api/memory/update/:id` | PUT | 更新记忆 |
| `/api/memory/delete/:id` | DELETE | 删除记忆 |
| `/api/memory/list` | GET | 列出记忆（分页） |

### 记忆迁移调度

- **持久化调度**: 每 5 分钟将 L1 记忆持久化到数据库
- **迁移调度**: 每小时检查并执行记忆层级迁移
- **向量嵌入**: 异步生成，不阻塞主流程
...
| `/api/learning/task/list` | GET | 列出学习任务 |
| `/api/learning/task/update/:id` | PUT | 更新学习任务状态 |
| `/api/learning/reflection/create` | POST | 创建反思 |
| `/api/learning/reflection/list` | GET | 列出反思 |
| `/api/learning/summary/daily` | GET | 获取每日学习总结 |
...
- `/api/collaboration/messages/history` - 获取历史消息
...
| `RUNTIME_API_KEY` | 运行时 API 密钥 | 生产环境必填 |
